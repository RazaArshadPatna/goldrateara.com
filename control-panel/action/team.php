...
function save_staff($conn, $name,$profession,$address,$mobile,$img,$entry_time,$entry_by,$qrdir,$status){

    
    $sql="INSERT INTO `team`(`name`,`profession`,`address`,`mobile`,`img`,`entry_time`,`entry_by`,`status`) VALUES(?,?,?,?,?,?,?,?)";
...
function update_staff($conn, $name,$profession,$address,$mobile,$img,$entry_time,$entry_by,$qrdir,$status,$id){
    ...
    $sql="update  `team` set `name`=?,`profession`=?,`address`=?,`mobile`=?,`img`=?, `entry_time`=?, `entry_by`=?,`status`=? where id=?";
