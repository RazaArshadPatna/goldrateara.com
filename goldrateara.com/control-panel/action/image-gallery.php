...
function save_homeSlider($conn,$image,$status,$entry_by,$entry_time){
    ...
    $sql="INSERT INTO `image_gallery`(`image`, `status`, `entry_by`, `entry_time`) VALUES (?,?,?,?)";
...
function update_homeSlider($conn,$image,$status,$entry_by,$entry_time,$id){ 
    ...
    $sql="update `image_gallery` set `image`=?, `status`=?, `entry_by`=?, `entry_time`=? where id=?";
...
if(isset($_POST['submit'])){
        ...
        if($p){
             ...
             ?>
             <script>location.href="../image-gallery.php"</script>
             <?php
